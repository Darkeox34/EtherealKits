...
public class FileStorage implements Storage {
    ...
    public FileStorage(Path dataFolder) {
        ...
        try {
            ...
            HeavenKits.getInstance().getLogger().severe("Failed to initialize local storage: " + e.getMessage());
        }
    }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(kitsDir, "*.yml")) {
            for (Path p : stream) {
                try {
                    ...
                    HeavenKits.getInstance().getLogger().warning("Failed to read kit file " + p.getFileName() + ": " + ex.getMessage());
                }
            }
        } catch (IOException e) {
            HeavenKits.getInstance().getLogger().severe("Failed to list kits directory: " + e.getMessage());
        }
    }
    ...
    private void writeKitFile(KitTemplate kit) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                HeavenKits.getInstance().getLogger().severe("Failed to write kit '" + kit.getName() + "': " + e.getMessage());
            }
        });
    }
    ...
    public void deleteKit(String name) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                HeavenKits.getInstance().getLogger().severe("Failed to delete kit '" + name + "': " + e.getMessage());
            }
        });
    }
    ...
    public void updatePlayerCooldown(String playerUUID, String kitName, long redeemTime) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                HeavenKits.getInstance().getLogger().severe("Failed to update cooldowns: " + e.getMessage());
            }
        });
    }
    ...
    public Map<String, Long> loadPlayerCooldowns(UUID playerUUID) {
        try {
            ...
            HeavenKits.getInstance().getLogger().severe("Failed to load player cooldowns: " + e.getMessage());
    ...
    public void removeExpiredCooldown(String playerUUID, String kitName) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                HeavenKits.getInstance().getLogger().severe("Failed to remove expired cooldown: " + e.getMessage());
            }
        });
    }
    ...
    public void cleanExpiredCooldowns() {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                HeavenKits.getInstance().getLogger().severe("Failed to clean expired cooldowns: " + e.getMessage());
            }
        });
    }
}